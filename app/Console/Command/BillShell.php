...
class BillShell extends Shell {
	...
	public function main() {
		...
		foreach ($companies as $c) {
			...
			//check if it should be run today
			$now = strtotime("now");
			$last_bill = strtotime($c['Company']['billing_last_run']);
			$diff = $now-$last_bill;
			
			$run = 0;
			switch ($s['Setting']['billing_period']) {
				case "weekly":
					if ($diff>601200) {
						$run = 1;	
					}
				...
				case "bi-weekly":
					...
					break;
				case "monthly":
					$month_diff = strtotime("now")-strtotime("-1 month")-3600;
					if ($diff>$month_diff) {
						$run = 1;	
					}
					break;
