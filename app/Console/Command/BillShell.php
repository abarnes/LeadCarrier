			...
			foreach ($vn as $v) {
				...
				if (count($records)>0) {
					...
					foreach ($records as $r) {
						...
						$lines[0] += array('line'=>array('name'=>'Lead','unit_cost'=>$s['Setting']['lead_price'],'quantity'=>'1','description'=>date('Y-m-d',strtotime($r['Record']['created'])).$r['Client']['first_name'].' '.$r['Client']['last_name']));
					}
					die(print_r($lines));
