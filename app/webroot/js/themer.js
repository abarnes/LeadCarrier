$(document).ready(function() {
	...
	$("div#mws-themer #mws-themer-toggle").bind("click", function(event) {
		if($(this).hasClass("opened")) {
			...
			$(this).toggleClass("opened").parent().animate({right: "250"}, "slow");
		}
	});
	...
	function attachStylesheet()
	{
		if($("#mws-stylesheet-holder").size() == 0) {
			$('head').append('<div id="mws-stylesheet-holder"></div>');
		}
		
		$("#mws-stylesheet-holder").html($('<style type="text/css">' + generateCSS() + '</style>'));
	}
